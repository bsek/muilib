...
void Zune::Group::addObject(Object *obj) {
    if (obj == NULL) {
        std::cerr << "RootClass is null, will not be added to group!" << std::endl;
    } else {
...
void Zune::Group::addObject(ZuneObject::RootObject &obj) {
    this->addObject(obj.getObject());
}

void Zune::Group::remObject(ZuneObject::RootObject &obj) {
    this->remObject(obj.getObject());
}
